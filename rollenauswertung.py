...
def _(dt, mo):
    ui_date = mo.ui.date(
        value=dt.date(year=dt.datetime.now().year, month=12, day=31)
        + dt.timedelta(days=1)
    )
    ui_date
    return (ui_date,)
...
def _(df, pl, ui_date):
    data = df.filter(
        ...
        pl.col("Ende").is_null() | pl.col("Ende").gt(ui_date.value),
...
def _(ORDNER_AUSGABE, data, data_einsatz_ist, data_uni, os, pl):
    data_uni.group_by(["Ortsteil", "Abteilung"]).agg(
        pl.col("FEUERnetz-ID").count()
    ).write_csv(os.path.join(ORDNER_AUSGABE, "personal_ges_uni_pivot.csv"))
    data_einsatz_ist.group_by(["Ortsteil", "Abteilung"]).agg(
        pl.col("FEUERnetz-ID").count()
    ).write_csv(os.path.join(ORDNER_AUSGABE, "personal_einsatz_ist_pivot.csv"))
    data.group_by(["Ortsteil", "Abteilung"]).agg(
        pl.col("FEUERnetz-ID").count()
