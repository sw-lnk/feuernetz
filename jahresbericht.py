...
def export_daten_jahresbericht(
    df: pl.DataFrame, datum_auswertung: dt.date, datum_befoerderung: dt.date
) -> None:
    befoerderungen = {
        "FFrA / FMA": "FMA",
        "FFr / FM": "FM",
    ...
    (
        ...
        .write_csv(
            os.path.join(
                db.ORDNER_AUSGABE, f"personalbewegung_{datum_auswertung.year}.csv"
            )
        )
    )
    ...
    for key_e, value_e in ehrungen.items():
        for jahr in value_e:
            df.filter(
                ...
                pl.col("Nachname"), pl.col("Vorname"), pl.col("Ortsteil")
            ).write_csv(
                os.path.join(
                    db.ORDNER_AUSGABE,
                    db.ORDNER_JAHRESBERICHT,
                    f"{key_e}-{jahr}Jahre.csv",
                )
            )
